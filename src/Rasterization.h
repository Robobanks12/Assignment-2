...
inline void DrawMesh(Image* image, Mesh mesh, const UniformData& data, FragmentShader fs)
{
	...
	for (size_t i = 0; i < mesh.count; i++)
	{
		...
		ndcs[i] = ndc;		// Resolution-agnostic pixel-position
		pixels[i] = screen;	// Screen-space pixel-position
	}
	...
	for (size_t face = 0; face < mesh.count / 3; face++)
	{
		for (int x = rects[face].xMin; x <= rects[face].xMax; x++)
		{
			...
			{
				size_t vertex = face * 3;
				Vector2 v0 = pixels[vertex + 0];
				Vector2 v1 = pixels[vertex + 1];
				Vector2 v2 = pixels[vertex + 2];
				...
				// Trilinear-interpolation formula is A * u + B * v + C * w
